		...
		try {
			...
			for (int i = 0; i < 2; ++i) {
				String sql = "insert into "
						+ "rsv_info(info_no, rsv_no, INFO_BOARDING_DATE, INFO_AIR_NM, INFO_DEPART_PLACE, INFO_DEPART_TIME, INFO_ARRIVE_PLACE, INFO_ARRIVE_TIME, INFO_PRICE, INFO_DIRECTION) "
				...
				// 
				pstmt3.setString(3, reserveInfo[i].getDepartPlace());
				// 출발 시간
				pstmt3.setString(4, reserveInfo[i].getDepartTime());
				// 도착 시간
				pstmt3.setString(5, reserveInfo[i].getArrivePlace());
				//
				pstmt3.setString(6, reserveInfo[i].getArriveTime());
				// 가격
				pstmt3.setString(7, reserveInfo[i].getPrice());
				// 가는 편 정보
				pstmt3.setString(8, reserveInfo[i].getDirection());
